...
import {
  Grid, Typography, Paper, CardMedia,
...
const EventsCompleted = ({ disableCreateEvent, classes, events }) => {
  ...
  const pastEvents = events.filter((event) => (event.dateEnd ? (
  ...
  pastEvents.map((event) => {
    ...
    const startOfWeek = moment(event.date).startOf('week');
    const endOfWeek = moment(event.date).endOf('week');
    const newWeek = { startOfWeek, endOfWeek, events: [event] };
    sortedEvents.push(newWeek);
  });

  sortedEvents = sortedEvents.sort((weekA, weekB) => (moment(weekA.startOfWeek).isBefore(weekB.startOfWeek) ? 1 : -1));

  // sort the events by date within week
  const sortedByDate = sortedEvents.map((weeklyEventsArray) => {
    const sortedEvents = weeklyEventsArray.events.sort((a, b) => (moment(a.date).isAfter(b.date) ? -1 : moment(a.date).isAfter(b.date) ? 1 : 0));
  ...
  return (
    <div className={classes.grid}>
      ...
      {sortedByDate[0].events.length > 0 && (
        <Typography
          gutterBottom
          variant="h5"
          style={{
