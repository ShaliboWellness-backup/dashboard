...
const Events = ({
  showLatestPastEvents, disableCreateEvent, classes, events,
}) => {
  ...
  let sortedByDate = sortedEvents.map((weeklyEventsArray) => {
    const sortedEvents = weeklyEventsArray.events.sort((a, b) => (moment(b.date).isAfter(a.date) ? -1 : moment(a.date).isAfter(b.date) ? 1 : 0));
    return { ...weeklyEventsArray, events: sortedEvents };
  });

  sortedByDate = sortedByDate.sort((a, b) => (a.startOfWeek.isBefore(b.startOfWeek) ? -1 : 1));

  const getTextForWeekHeader = (startOfWeek) => {
    const diff = moment().startOf('week').diff(startOfWeek, 'weeks');
